...
module.exports.BlogCategory = {
    ...
    read: async (req, res) => {
        const data = await BlogCategory.findOne({ _id: req.params.categoryId }).populate('blogCategoryId')
        res.status(202).send({
            error: false,
            data: data
        })
...
module.exports.BlogPost = {
    ...
    list: async (req, res) => {
        ...
        // console.log(search);
         //* { title: 'test', content: 'test' } bu ifade bir isime yaramaz, soyle olmali --> { title: { $regex: 'test' }, content: { $regex: 'test' } }
         
        for (let key in search) {
            // search[key] = { $regex: search[key] }   //* regex i kullanma amacimiz icinde arama yapmasini saglamak.
        ...
        //? Limit:   ------> Sayfada kac adet veri getirecegini belirtir.
        let limit = Number(req.query?.limit)
        limit = limit > 0 ? limit : Number(process.env.PAGE_SIZE || 20)  //* .env de ki butun degerler string oldugundan numn=ber a donusturduk.
        console.log('limit', limit)

        //? Page:   ----->  Kac sayfa atlayacagimi soyleyecek.
        let page = Number(req.query?.page)
        // page = page > 0 ? page : 1     //* page in defaultu 1 dir.
        page = page > 0 ? (page - 1) : 0  //* Backend 'de sayfa sayısı her zmaan page-1 olarak hesaplanmalı.
        console.log('page', page)

        //? Skip:  -------->  Kac kaydi atlayacagimi soyleyecek.
        // LIMIT 20, 10
        ...
        // const data = await BlogPost.find().populate('blogCategoryId')   //*blogCategoryId yi sadece id olarak degil ilgili category de ki detaylari ile birlikte goruyoruz.
        const data = await res.getModelList(BlogPost, 'blogCategoryId')   //* getModelList e 2.parametre de gonderdik.
